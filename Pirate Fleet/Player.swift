...
class Player {
    ...
    func addPlayerShipsMinesMonsters(numberOfMines: Int = 0, numberOfSeamonsters: Int = 0) {
        ...
        for _ in 0..<numberOfMines {
            ...
            var mine = Mine(location:GridLocation(x:location.x,y:location.y), penaltyText: "You hit a mine", guaranteesHit: true)
            while !gridViewController.addMine(mine, playerType: .Computer) {
                location = RandomGridLocation()
                mine = Mine(location:GridLocation(x:location.x,y:location.y), penaltyText: "You hit a mine", guaranteesHit: true)
            }
        }
        ...
        for _ in 0..<numberOfSeamonsters {
            var location = RandomGridLocation()
            var seaMonster = SeaMonster(location:GridLocation(x:location.x,y:location.y),guaranteesHit: true, penaltyText: "Sea Monster here")
            while !gridViewController.addSeamonster(seaMonster, playerType: .Computer) {
                location = RandomGridLocation()
                seaMonster = SeaMonster(location:GridLocation(x:location.x,y:location.y),guaranteesHit: true, penaltyText: "Sea Monster here")
            }
        }
    }
}